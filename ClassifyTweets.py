...
gws = [28, 29, 30, 31]
teams = []
sqlquery = "SELECT ShortTeamName FROM pp_teams"
...
for gw in gws:
    for team in teams:
        sqlstmt = "SELECT * FROM tweetdata WHERE GameWeek = '%s' AND Team = '%s'" % (gw, team)
        ...
        if results:
            ...
            query = "INSERT INTO pp_sentimentscore (GameWeek, TeamName) VALUES(%s,'%s')" % (gw, team)
            ...
            for model in models:
                ...
                if count != 0:
                    ...
                    query = "UPDATE pp_sentimentscore SET %s = %s WHERE GameWeek = %s AND TeamName = '%s'" % (model['Name'], clf_score[0], gw, team)
                    cursor.execute(query)
                    conn.commit()
    #except:
